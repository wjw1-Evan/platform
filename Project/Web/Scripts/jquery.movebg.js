(function($){
	...
	var defaultser=$.extend(defaults,options);
	    return this.each(function() {
	        ...
	        _mover.css({ left: "" + ((defaultser.width + 4) * origin +10) + "px" }); /*设置滑块当前显示的位置*/
	        ...
	        function move() {
	            ...
	            _mover.queue(
	                function() {
	                    $(this).show().stop(true, true).animate({ left: "" + Number(cur * (defaultser.width+4)+10+ extra) + "" }, defaultser.speed),
	                        function() { $(this).dequeue() }
	                }
	            );
	            _mover.queue(
	                function() {
	                    $(this).stop(true, true).animate({ left: "" + (cur * (defaultser.width + 4) +10) + "" }, defaultser.rebound_speed),
	                        function() { $(this).dequeue() }
	                }
	            );
	        };
