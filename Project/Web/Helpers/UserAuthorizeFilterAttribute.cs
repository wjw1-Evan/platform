...
namespace Web.Helpers
{
    ...
    public class UserAuthorizeAttribute : AuthorizeAttribute
    {
        ...
        protected override bool AuthorizeCore(HttpContextBase httpContext)
        {
            ...
            //检测用户是否有权限访问 
            if (!sysRoleService.CheckSysUserSysRoleSysControllerSysActions(userInfo.UserId, area, action, controller).Result)
                throw new AuthenticationException("用户：" + userInfo.UserName + "(" + userInfo.UserId + ") 没有权限访问 " + area + " > " + controller + " > " + action + " ！请联系系统管理员进行权限分配！");
